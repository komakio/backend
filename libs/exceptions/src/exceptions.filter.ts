...
export class ExceptionsFilter implements ExceptionFilter {
    ...
    private catchRabbitMQException(exception: HttpException | Error, context: RmqContext) {
    ...
    private catchApiException(exception: HttpException | Error, host: ArgumentsHost) {
        const ctx = host.switchToHttp();
        ...
        if ((exception as AxiosError).isAxiosError) {
            ...
            this.exceptions.report(new Error(`${method} ${url} failed: ${status} ${statusText}`), {
                ...baseInfo,
                isAxios: true,
                url,
                method,
